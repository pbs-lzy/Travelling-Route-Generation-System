...
def read_csv(city_name):
    csv_file = open("D:/DevelopTest/ProjectTest/TravelPlace/data/city_list.csv", "r")
...
def parse_html(html):
    ...
    for div in divs:
        ...
        page_location.append(result['location'])
        page_address.append(result['address'])
    return page_title, page_location, page_address
...
def get_city_places(city_name):
    ...
    for index_url in get_index_url(city_name, num_pages + 1):
        ...
        if html:
            page_title, page_location, page_address = parse_html(html)
            all_titles.extend(page_title)
            all_location.extend(page_location)
            all_addresses.extend(page_address)
    return all_titles, all_location, all_addresses
